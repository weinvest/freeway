...
public:
    ...
    void * GetValue(WorkflowID_t flowID)
    {
        printf("===========>Enter B2::GetValue  flowID=%d\n", flowID); 
        sprintf(mValue, "B2::GetValue flow=%d", flowID);
        return  mValue;
    }
...
public:
    ...
    void * GetValue(WorkflowID_t flowID)
    {
        printf("===========>Enter B3::GetValue  flowID=%d\n", flowID); 
        sprintf(mValue, "B3::GetValue flow=%d", flowID);
        return  mValue;
    }
...
public:
    ...
    int32_t DoProcess(WorkflowID_t workflowId)
    {
        ...
        for(auto preNode : mPrecursors)
        {
            RemotePtr<DEventNode> RemoteCall(preNode);
            char *pData = static_cast<char*>(RemoteCall->GetValue(workflowId));
        }
#endif
...
//Task Test!!!
    ...
    std::vector<DEventNode*> NodeManager{pA1, pB1, pB2, pB3, pC1};
